...
def strong_to_weak_filter_ok(direction: int, swing_bias: int, trailing: Trailing) -> bool:
    # Require trailing extremes exist
    if trailing.top is None or trailing.bottom is None:
        return False
    # LONG only when Strong Low -> Weak High (swing bullish)
    if direction == BULLISH:
        return swing_bias == BULLISH
    # SHORT only when Strong High -> Weak Low (swing bearish)
...
# BACKTEST ENGINE
# L1:
#   if last OB destroyed, and NEXT OB is opposite and intersects -> trade by next OB
# L2:
#   if new OB intersects any active FVG with same bias -> trade,
#   PLUS filter strong->weak direction (added)
# =============================
def run_backtest(df: pd.DataFrame, df4h: pd.DataFrame, ltf_sec: int):
    ...
    for i in range(n):
        ...
        # FVG mitigate then maybe create on new 4h
        mitigate_fvgs(i)
        maybe_create_fvg_on_new_4h(i)

        # detect new internal OB(s)
        ...
        if i > 0:
            ...
            # swingTrend update (for strong->weak filter)
        ...
        # entries (flat only, need next bar)
        if i >= n - 1:
            continue
        if pos is not None:
            continue
        ...
        if waiting_replacement and destroyed_ob is not None:
            ...
            # "NEXT OB" didn't match -> reset waiting (per your wording)
            waiting_replacement = False
            destroyed_ob = None

        # L2: OB intersects FVG same dir + strong->weak filter (added)
        opened = False
        for ob in new_obs:
            direction = BULLISH if ob.bias == BULLISH else BEARISH

            # FILTER: must match strong->weak direction
            if not strong_to_weak_filter_ok(direction, swingTrend.bias, trailing):
                continue
            ...
            qty = (equity * RISK_PCT) / risk_per_unit
            pos = Position(direction=direction, entry=entry_price, qty=qty, stop=stop, take=float(take),
                           entry_time=float(t[i + 1]), reason="L2_new_ob_intersects_fvg_same_direction_strong_to_weak")
            l2_count += 1
            opened = True
            break
...
class BacktestWindow(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Backtest 6m: L1 destroyed->next OB opposite+intersect | L2 OB∩FVG same dir + Strong->Weak filter")
    ...
    def recalc(self):
        ...
        out, trades, l1c, l2c = run_backtest(df, df4h, ltf_sec=ltf_sec)
