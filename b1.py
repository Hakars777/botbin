...
# Volatility parsing (Lux-style)
ATR_LEN = 200
HIGH_VOL_MULT = 2.0  # (high-low) >= 2*ATR => swap high/low

# FVG (4h, auto threshold, extend=5)
FVG_TF = "4h"
FVG_EXTEND = 5

# Backtest params (kept from txt2)
START_EQUITY = 1000.0
RISK_PCT = 0.01
...
def utc_months_ago_ms(months: int) -> int:
...
def zones_intersect(a_lo: float, a_hi: float, b_lo: float, b_hi: float) -> bool:
    a0, a1 = (a_lo, a_hi) if a_lo <= a_hi else (a_hi, a_lo)
    b0, b1 = (b_lo, b_hi) if b_lo <= b_hi else (b_hi, b_lo)
...
def almost_same_ob(a: "OrderBlock", b: "OrderBlock", eps: float = 1e-9) -> bool:
    return (
        abs(a.barHigh - b.barHigh) <= eps and
        abs(a.barLow - b.barLow) <= eps and
        abs(a.barTime - b.barTime) <= eps and
        a.bias == b.bias
    )


# =============================
# REST: klines paging by startTime
# =============================
def fetch_klines_from(symbol: str, tf: str, start_ms: int, end_ms: Optional[int] = None) -> pd.DataFrame:
...
# =============================
# LuxAlgo-like volatility parsing
# =============================
def true_range(h: np.ndarray, l: np.ndarray, prev_c: np.ndarray) -> np.ndarray:
...
class FVG:
    ...
    bias: int  # +1 bull, -1 bear
    leftTime: float
    rightTime: float  # already extended (drawing / time validity)
...
def strong_to_weak_filter_ok(direction: int, swing_bias: int) -> bool:
    # txt2 filter logic; bias comes from swing structure state at bar i
    if direction == BULLISH:
        return swing_bias == BULLISH
    return swing_bias == BEARISH


# =============================
# Backtest Engine (no lookahead FVG, aligned to txt1)
# =============================
def run_backtest(df: pd.DataFrame, df4h: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, int, int]:
    ...
    def store_internal_order_block(pivot: Pivot, bias: int, cur_bar_i: int) -> Optional[OrderBlock]:
        if pivot.barIndex is None:
            return None
        a = int(pivot.barIndex)
        b = int(cur_bar_i)  # slice end exclusive (Pine)
        if b <= a + 1:
            return None
        if a < 0 or b > n:
            return None
        ...
        if len(internalOrderBlocks) > 300:
            internalOrderBlocks.pop()
        return ob

    def mitigate_internal_obs(cur_bar_i: int) -> List[OrderBlock]:
        curH = float(h[cur_bar_i])
        curL = float(l[cur_bar_i])
        kept: List[OrderBlock] = []
        removed: List[OrderBlock] = []
    ...
    def mitigate_fvgs(cur_bar_i: int):
        cur_low = float(l[cur_bar_i])
        cur_high = float(h[cur_bar_i])
        for k in range(len(active_fvgs) - 1, -1, -1):
            g = active_fvgs[k]
            if (g.bias == BULLISH and cur_low < g.bottom) or (g.bias == BEARISH and cur_high > g.top):
                active_fvgs.pop(k)

    # ---- FVG stream state (no lookahead; matches txt1 behavior)
    ...
    htf_sec = interval_seconds(FVG_TF)
    extend_sec = float(FVG_EXTEND * base_tf_sec)

    cum_abs = 0.0  # same state as txt1/2

    # LTF-mode state
    prev_j_ltf = -1
    cur_htf_high: Optional[float] = None
    cur_htf_low: Optional[float] = None
    cur_htf_open_time: Optional[float] = None

    # HTF/equal-mode state (add only fully closed 4h candles)
    prev_k_closed = -1
    t4_close = t4 + float(htf_sec)

    def fvg_add(bullish: bool, bearish: bool, currentHigh: float, currentLow: float,
                lastClose: float, lastOpen: float, lastTime: float, currentTime: float,
                last2High: float, last2Low: float, barDeltaPercent: float):
        nonlocal cum_abs
        if bullish:
            active_fvgs.insert(0, FVG(
                top=float(currentLow),
                bottom=float(last2High),
                bias=BULLISH,
                leftTime=float(lastTime),
                rightTime=float(currentTime + extend_sec),
            ))
        if bearish:
            active_fvgs.insert(0, FVG(
                top=float(currentHigh),
                bottom=float(last2Low),
                bias=BEARISH,
                leftTime=float(lastTime),
                rightTime=float(currentTime + extend_sec),
            ))
        if len(active_fvgs) > 500:
            del active_fvgs[500:]

    def maybe_update_fvgs(i: int):
        """
        No-lookahead policy:
        - If base_tf < 4h: emulate txt1 LTF mode (developing 4h candle built from LTF bars), add FVG only on new 4h boundary.
        - If base_tf >= 4h: add FVGs only when 4h candles are fully closed by the close of current base bar.
        """
        nonlocal prev_j_ltf, cur_htf_high, cur_htf_low, cur_htf_open_time, cum_abs, prev_k_closed

        if t4.size < 5:
            return

        if base_tf_sec < htf_sec:
            # LTF mode: build the CURRENT 4h candle from LTF bars (no lookahead), add only at boundary (txt1).
            ...
            # Update developing 4h extrema using available LTF bars up to i
            if new_tf or cur_htf_open_time is None:
                cur_htf_open_time = float(t4[j])
                cur_htf_high = float(h[i])
                cur_htf_low = float(l[i])
            else:
                hi = float(h[i])
                lo = float(l[i])
                if cur_htf_high is None or hi > cur_htf_high:
                    cur_htf_high = hi
                if cur_htf_low is None or lo < cur_htf_low:
                    cur_htf_low = lo
            ...
            if new_tf and j >= 2:
                ...
                # current developing 4h candle values as-of bar i (no future)
                currentHigh = float(cur_htf_high if cur_htf_high is not None else h[i])
                currentLow = float(cur_htf_low if cur_htf_low is not None else l[i])
                currentTime = float(t4[j])  # open time of current 4h candle
                ...
                fvg_add(bullish, bearish, currentHigh, currentLow,
                        lastClose, lastOpen, lastTime, currentTime,
                        last2High, last2Low, barDeltaPercent)

            prev_j_ltf = j
            return

        # HTF/equal mode: add only fully closed 4h candles by base-bar close.
        # Base-bar close time:
        base_close_time = float(t[i] + float(base_tf_sec))

        # last fully closed 4h candle index:
        k_closed = int(np.searchsorted(t4_close, base_close_time, side="right") - 1)
        if k_closed <= prev_k_closed:
            return

        # Add sequentially (no skipping), strictly closed candles only:
        for k in range(prev_k_closed + 1, k_closed + 1):
            if k < 2 or k >= t4.size:
                continue

            lastClose = float(c4[k - 1])
            lastOpen = float(o4[k - 1])
            lastTime = float(t4[k - 1])

            # candle k is fully closed here
            currentHigh = float(h4[k])
            currentLow = float(l4[k])
            currentTime = float(t4[k])  # open time

            last2High = float(h4[k - 2])
            last2Low = float(l4[k - 2])

            denom = (lastOpen * 100.0)
            barDeltaPercent = ((lastClose - lastOpen) / denom) if abs(denom) > 1e-12 else 0.0

            cum_abs += abs(barDeltaPercent)
            denom_idx = float(k) if k > 0 else 1.0
            threshold = (cum_abs / denom_idx) * 2.0
        ...
        prev_k_closed = k_closed

    # ---- L1 replacement state (txt2)
    waiting_replacement = False
    destroyed_ob: Optional[OrderBlock] = None
    ...
    # MAIN LOOP (bar-close logic, entry at next open)
    for i in range(n):
        # 1) Handle exits on bar i (uses OHLC of bar i; standard bar simulation)
        if pos is not None:
            hi = float(h[i])
            lo = float(l[i])
            exit_price = None
        ...
        # 2) Update pivots/structure/trailing (txt1 order)
        ...
        # 3) Mitigate internal OBs FIRST (same placement as txt2; ok for signals)
        pre_top_ob = internalOrderBlocks[0] if internalOrderBlocks else None
        removed_obs = mitigate_internal_obs(i)
        if pre_top_ob is not None:
            if any(almost_same_ob(r, pre_top_ob) for r in removed_obs):
                destroyed_ob = pre_top_ob
                waiting_replacement = True

        # 4) FVG mitigation then maybe create (no lookahead, aligned to txt1)
        mitigate_fvgs(i)
        maybe_update_fvgs(i)

        # 5) Detect new internal OB(s) (txt1 logic), and update swingTrend bias (for filter)
        new_obs: List[OrderBlock] = []
        if i > 0:
            prev_close = float(c[i - 1])
            cur_close = float(c[i])

            # Internal structure -> store internal OBs
            if internalHigh.currentLevel is not None and not internalHigh.crossed:
                if crossover(prev_close, cur_close, float(internalHigh.currentLevel)):
                    ...
                    ob = store_internal_order_block(internalHigh, BULLISH, i)
                    if ob is not None:
                        new_obs.append(ob)
            ...
            if internalLow.currentLevel is not None and not internalLow.crossed:
                if crossunder(prev_close, cur_close, float(internalLow.currentLevel)):
                    ...
                    ob = store_internal_order_block(internalLow, BEARISH, i)
                    if ob is not None:
                        new_obs.append(ob)

            # Swing trend update (BOS/CHOCH tags not needed in backtest, only bias)
            if swingHigh.currentLevel is not None and swingHigh.barIndex is not None and not swingHigh.crossed:
                if crossover(prev_close, cur_close, float(swingHigh.currentLevel)):
            ...
            if swingLow.currentLevel is not None and swingLow.barIndex is not None and not swingLow.crossed:
                if crossunder(prev_close, cur_close, float(swingLow.currentLevel)):
                    swingLow.crossed = True
                    swingTrend.bias = BEARISH

        # 6) Entries (flat only), enter at next bar open (no lookahead)
        if i >= n - 1:
            continue
        if pos is not None:
            continue
        ...
        # L1: destroyed -> NEXT OB opposite + intersects -> trade
        if waiting_replacement and destroyed_ob is not None:
            first_ob = new_obs[0]
            if (first_ob.bias != destroyed_ob.bias) and zones_intersect(
                first_ob.barLow, first_ob.barHigh, destroyed_ob.barLow, destroyed_ob.barHigh
            ):
                ...
                if take is not None:
                    ...
                    if risk_per_unit > 0:
                        ...
                        pos = Position(
                            ...
                            stop=stop,
                            take=float(take),
                            entry_time=float(t[i + 1]),
                            reason="L1_destroyed_then_next_ob_opposite_and_intersects",
                        )
                        l1_count += 1
                        waiting_replacement = False
                        destroyed_ob = None
            ...
            # Next OB didn't match -> reset (per txt2 wording)
            waiting_replacement = False
            destroyed_ob = None

        # L2: new OB intersects active FVG same dir + strong->weak direction filter
        for ob in new_obs:
            direction = BULLISH if ob.bias == BULLISH else BEARISH

            if not strong_to_weak_filter_ok(direction, swingTrend.bias):
                continue
            ...
            pos = Position(
                ...
                take=float(take),
                entry_time=float(t[i + 1]),
                reason="L2_new_ob_intersects_fvg_same_direction_strong_to_weak",
            )
            l2_count += 1
            break
...
class BacktestWindow(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle(
            "Backtest: L1 destroyed->next OB opposite+intersect | L2 OB∩FVG same dir + Strong->Weak | FVG no-lookahead (txt1)"
        )
    ...
    def recalc(self):
        ...
        self.status.setText(f"loading last {HIST_MONTHS} months...")
        QtWidgets.QApplication.processEvents()

        start_ms = utc_months_ago_ms(HIST_MONTHS)
        end_ms = utc_now_ms()

        # Base TF
        df = fetch_klines_from(SYMBOL, tf, start_ms, end_ms)

        # Always fetch 4h (and a bit earlier for stable mapping)
        pad_ms = 20 * interval_seconds("4h") * 1000
        df4h = fetch_klines_from(SYMBOL, "4h", start_ms - pad_ms, end_ms)

        if len(df) < 800 or len(df4h) < 50:
